...
public:
    ...
    void Put(T const& a, Args&&... bs) {
        ((put_one(a, std::forward<Args>(bs))), ...);
    }
    template <typename... Args>
    void PutReverse(T const& b, Args&&... as) {
...
public:
    ...
    TermType BestMatch(TermType const& term) { return minimum_superior(term); }
...
private:
    ...
    POCompareResult lookup(TermTree* tree, TermType const& new_term, TermTree** node) {
