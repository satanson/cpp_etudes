...
int64_t exp10s[] = {
        Exp10<0>::value,  Exp10<1>::value,  Exp10<2>::value,  Exp10<3>::value,  Exp10<4>::value,
        Exp10<5>::value,  Exp10<6>::value,  Exp10<7>::value,  Exp10<8>::value,  Exp10<9>::value,
        Exp10<10>::value, Exp10<11>::value, Exp10<12>::value, Exp10<13>::value, Exp10<14>::value,
        Exp10<15>::value, Exp10<16>::value, Exp10<17>::value, Exp10<18>::value,
};
...
struct bit64_traits {};
template <>
struct bit64_traits<int64_t> {
    typedef int64_t type;
};
template <>
struct bit64_traits<uint64_t> {
...
struct DorisDecimalOp {
    ...
    static constexpr int128_t MAX_DECIMAL_VALUE = static_cast<int128_t>(MAX_INT_VALUE) * ONE_BILLION + MAX_FRAC_VALUE;
    static constexpr int128_t MIN_DECIMAL_VALUE = -MAX_DECIMAL_VALUE;

    template <class T, class U, class TT = typename bit64_traits<T>::type, class UU = typename bit64_traits<U>::type>
...
struct CKDecimalOp {
    ...
    int128_t div(int128_t a, int128_t b, int128_t scale) {
        if constexpr (check_overflow) {
            bool overflow = false;
            if constexpr (!is_decimal_a) overflow |= mulOverflow(scale, scale, scale);
            overflow |= mulOverflow(a, scale, a);
            if (overflow) return static_cast<int128_t>(-1);
        } else {
...
struct PrepareData {
    ...
    PrepareData() {
        ...
        std::cout << std::boolalpha << "prepare data: batch_size=" << batch_size << ", fill_zero=" << fill_zero
                  << ", fill_max=" << fill_max << std::endl;
...
static inline std::string ToHexString(int128_t x) {
    ...
    std::stringstream ss;
    ss << std::hex << std::showbase << "{high=" << wx.s.high << ", low=" << wx.s.low << "}";
    return ss.str();
}
