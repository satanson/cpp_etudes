...
void concat_vertically(Columns &columns, std::vector<uint8_t> &buffer, std::vector<uint32_t> &slice_sizes) {
  ...
  for (const auto &key_column : columns) {
    key_column->serialize_batch(buffer.data(), slice_sizes, columns[0]->size(), max_one_row_size);
  }
}
