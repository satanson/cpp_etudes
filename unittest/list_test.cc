...
public:
    void multi_thread_run(MichaelList& list, size_t thread_nr, ThreadFunc thread_func, CheckFunc check_func) {
        assert(thread_nr > 0);
    ...
    void multi_thread_run_same_key(std::shared_ptr<MichaelList> list, size_t thread_nr, size_t rounds, uint32_t key,
                                   SameKeyThreadFunc thread_func, SameKeyCheckFunc check_func) {
        std::vector<std::thread> threads;
        threads.reserve(thread_nr);
        for (auto i = 0; i < thread_nr; ++i) {
