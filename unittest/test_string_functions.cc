...
TEST_F(TestStringFunctions, compare_utf8_length3) {
    compare([](std::string const& s) { ASSERT_EQ(StringFunctions::utf8_length(s), StringFunctions::utf8_length3(s)); });
}
...
TEST_F(TestStringFunctions, substr_ascii) {
    ...
    std::vector<std::tuple<int, int, std::string>> cases = {
            {1, 2, "12"},    {1, 0, ""},      {2, 100, "23456789"}, {9, 1, "9"},           {9, 100, "9"},
            {10, 1, ""},     {-9, 1, "1"},    {-9, 9, "123456789"}, {-9, 10, "123456789"}, {-4, 1, "6"},
            {-4, 4, "6789"}, {-4, 5, "6789"}, {-1, 1, "9"},         {-1, 2, "9"}};
    for (auto& e : cases) {
...
TEST_F(TestStringFunctions, substr_zh) {
    ...
    std::vector<std::tuple<int, int, std::string>> cases = {{1, 2, "壹贰"},
                                                            {1, 0, ""},
                                                            {2, 100, "贰叁肆伍陆柒捌玖"},
                                                            {9, 1, "玖"},
                                                            {9, 100, "玖"},
...
TEST_F(TestStringFunctions, upper) {
    ...
    for (auto& s : cases) {
        ASSERT_EQ(StringFunctions::upper_new(StringFunctions::lower_new(StringFunctions::upper_new(s))),
