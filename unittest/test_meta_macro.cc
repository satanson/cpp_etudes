...
#define M_DEF_STMT(type, ...)                                                  \
  META_MACRO_SELECT(M_DEF_STMT_, 1, type, __VA_ARGS__)
...
#define M_MODIFIER_DEF_STMT(modifier, type, ...)                               \
  META_MACRO_SELECT(M_MODIFIER_DEF_STMT_, 2, modifier, type, __VA_ARGS__)
...
template <typename T1, typename T2, typename T3> struct IsBinaryFunction {
  static constexpr bool value = false;
};
...
#define IS_BINARY_FUNCTION_CTOR(a, b, c)                                       \
  template <> struct IsBinaryFunction<a, b, c> {                               \
