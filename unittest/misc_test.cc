...
TEST_F(MiscTest, testTuple) {
  std::tuple<std::string, std::string> t0 =
      std::make_tuple(std::string(4096, 'x'), std::string(4096, 'y'));
  std::tuple<std::string, std::string> t1 =
      std::make_tuple(std::get<1>(t0), std::get<0>(t0));
...
template <bool flag> class ConstructorParamsWithMaybeUnusedModifier {
  int c;

public:
  ConstructorParamsWithMaybeUnusedModifier([[maybe_unused]] int a,
                                           [[maybe_unused]] int b) {
    if constexpr (flag) {
      c = a + b;
    } else {
...
template <typename T> void func_tmpl(FloatGuard<T> a) {
  std::cout << "float-typed func: " << a << std::endl;
}

TYPE_GUARD(IntegerGuard, type_is_integer, int8_t, uint8_t, int16_t, uint16_t,
           int32_t, uint32_t, int64_t, uint64_t)
template <typename T> void func_tmpl(IntegerGuard<T> a) {
