...
template <typename... Args> MyInt add0(Args &&...args) { return (args + ...); }

template <typename... Args> MyInt add1(Args &&...args) { return (... + args); }
...
TEST_F(TestCpp17, testLongLong) { std::cout << sizeof(__int128) << std::endl; }
namespace common {
template <typename T> inline bool mulOverflow(T x, T y, T &res) {
  std::cout << "call generic mulOverflow" << std::endl;
  return __builtin_mul_overflow(x, y, &res);
}

template <> inline bool mulOverflow(int x, int y, int &res) {
  return __builtin_smul_overflow(x, y, &res);
}

template <> inline bool mulOverflow(long x, long y, long &res) {
  return __builtin_smull_overflow(x, y, &res);
}

template <> inline bool mulOverflow(long long x, long long y, long long &res) {
