...
void BM_utf8_length(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length(s); });
    }
}

void BM_utf8_length2(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length2(s); });
    }
}

void BM_utf8_length3(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length3(s); });
    }
}

void BM_utf8_length4(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length4(s); });
    }
}
void BM_utf8_length_simd_sse2(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length_simd_sse2(s); });
    }
}
void BM_utf8_length_simd_avx2(benchmark::State& state) {
    for (auto _ : state) {
        apply_vector(data, result, [](std::string& s) { return StringFunctions::utf8_length_simd_avx2(s); });
    }
}
