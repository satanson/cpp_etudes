...
static void BM_Int128_And(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs.data(), rhs.data(), result.data(), [](auto x, auto y) { return x & y; });
}

static void BM_Int128_Shift(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs.data(), rhs.data(), result.data(), [](auto x, auto y) { return x >> 16; });
}
...
static void BM_Int128_Mul(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs.data(), rhs.data(), result.data(), [](int128_t x, int128_t y) { return x * y; });
}
...
static void BM_Int128_DivMod(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs.data(), rhs.data(), result.data(),
                       [](int128_t x, int128_t y) { return (x / y) & (x % y); });
}

static void BM_Int64_And(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs64.data(), rhs64.data(), result64.data(), [](auto x, auto y) { return x & y; });
}

static void BM_Int64_Shift(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs64.data(), rhs64.data(), result64.data(), [](auto x, auto y) { return x >> 16; });
}
...
static void BM_Int64_Mul(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs64.data(), rhs64.data(), result64.data(),
...
static void BM_Int32_And(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs32.data(), rhs32.data(), result32.data(), [](auto x, auto y) { return x & y; });
}

static void BM_Int32_Shift(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs32.data(), rhs32.data(), result32.data(), [](auto x, auto y) { return x >> 16; });
}
...
static void BM_Int32_Mul(benchmark::State& state) {
    for (auto _ : state)
        single_compute(batch_size, lhs32.data(), rhs32.data(), result32.data(),
