...
for my $d (@$drivers) {
  ...
  $d->{EffectiveSchedRatio}=$d->{ScheduleCounter}==0?1:($d->{ScheduleEffectiveCounter}+0.0)/$d->{ScheduleCounter};
}
...
my $index="EffectiveSchedRatio";
print join "\n", map {sprintf "%0.3f\t%s\tschedule_times(%d/%d)\t%s", $_->{$index}, $index, $_->{ScheduleEffectiveCounter}, $_->{ScheduleCounter}, $_->{id}} sort{$b->{$index}<=>$a->{$index}} grep{$_->{ScheduleCounter}>$limit} @$drivers;
print "\n";
