...
sub ensure_ag_installed(){
  my ($ag_path)=map{chomp;$_} qx(which ag 2>/dev/null);
...
sub all_sub_classes(){
  ...
  if ((-f $cache_file) && (((-M $cache_file)*3600*24) < 600)) {
    @matches=map {chomp; $_} qx(cat $cache_file);
    qx(touch $cache_file);
  } else {
    @matches=map {chomp;$_} qx(ag -G '\.(c|cc|cpp|C|h|hh|hpp|H)\$' '$cls_re');
    @matches=merge_lines @matches;
    @matches=map {s/$attr_re//g; $_} @matches;
    @matches=map {s/$access_specifier_re//g; $_} @matches;
    @matches=map {s/$template_arguments_re//g; $_} @matches;
    @matches=map {s/$template_arguments_re//g; $_} @matches;
    @matches=map {s/$template_arguments_re//g; $_} @matches;
    @matches=map {s/$template_arguments_re//g; $_} @matches;
    @matches=map {s/template//g; $_} @matches;
    @matches=map {s/^(.+?)\s*{/$1/g; $_} @matches;
    @matches=map {s/\s+/ /g; $_} @matches;
  ...
  my %table=map{$_->[1]=>$_} grep {defined($_)} map{ if (/^(\S+)\s*:\s*$class0_re/){[$1,$2]}else{print "undefined:$_\n"; undef}} @matches;
  #print Dumper(\%table);
  ...
  for my $re (@class_re_list){
    my @parent_matches=grep {defined($_)} map{ if (/^(\S+)\s*:\s*$re\s*$/){[$1,$2,$3]}else{undef}} @matches;
