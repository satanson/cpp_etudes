...
public:
    ...
    bool starts_with(const Slice& x) const { return ((size >= x.size) && memcmp(data, x.data, x.size) == 0); }

    bool ends_with(const Slice& x) const {
        return ((size >= x.size) && memcmp(data + (size - x.size), x.data, x.size) == 0);
    }
...
inline int Slice::compare(const Slice& b) const {
    auto min_size = std::min<size_t>(size, b.size);
    auto r = memcmp(data, b.data, min_size);
    return r != 0 ? r : size - b.size;
}
